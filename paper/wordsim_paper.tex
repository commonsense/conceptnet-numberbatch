...
using machine-learning algorithms that attempt to represent words or phrases as vectors in a high dimensional space,
...
\begin{tikzpicture}
    \begin{axis}[enlargelimits=0.2, xlabel={MEN-3000 (common words)}, ylabel={RW (rare words)}]
        \addplot[
          ...
          point meta=explicit symbolic, every node near coord/.append style={anchor=west}
        ]
        table[meta=label] {
            x      y     label
            .777   .514  {Levy SVD}
            .774   .470  {Levy skip-gram}
            .816   .476  {GloVe}
            .840   .512  {WL0}
            .860   .585  {WL1}
            .860   .589  {WL2}
...
\end{tikzpicture}
\caption{
    Systems discussed in this paper, plotted according to their MEN-3000 and
    RW scores.
}
\label{compare-graph}
\end{figure}
