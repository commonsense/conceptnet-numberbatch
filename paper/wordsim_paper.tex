...
As a practical effect of this, Table~\ref{eval-variations} shows that expanded
retrofitting with self-loops added to the diagonal performs better on word
similarity evaluations than it does without, when allowed to run for 10 steps
of retrofitting.
...
\midrule
% FIXME: rerun this with new data!
{\bf Unmodified}                     & \bf .587 & \bf .858 & \bf .818 \\
Use first row instead of row-merging & .563 &      .827 &    .787 \\
Unweighted row-merging               & .526 &      .844 &    .751 \\
No self-loops in retrofitting        & .570 &      .855 &    .790 \\
\bottomrule
\end{tabular}
\caption{
...
Our full ensemble with all six components performs better than the previously
published systems: we can reject the null hypothesis that our ensemble performs
the same as one of these published systems with $p < .01$.  It is inconclusive
whether it is better to include or exclude PPDB in the ensemble, as the results
with and without it are very close.
...
Some of the procedures we implemented in creating the ensemble systems
require some justification. To show the benefits of certain decisions,
...
We have tagged this revision as {\em submitted-20160309}, so that this paper
can be reproduced as-is, even if we update the code later.

The matrix of embeddings for the full ensemble can also be downloaded from
Amazon S3. The matrix values in NumPy form are located at [TODO], and the
ConceptNet-normalized labels corresponding to the rows of this matrix are
located at [TODO].
...
We thank the word2vec and GloVe teams for opening their data so that new
techniques can be built on top of their results. We also thank the tens of
thousands of pseudonymous contributors to Wiktionary, Open Mind Common Sense,
and related projects, for their frequently uncredited work in providing
freely-available lexical knowledge.
