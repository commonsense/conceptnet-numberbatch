...
\maketitle
\begin{abstract}
A currently successful approach to
represent the meanings of words is to represent them as vectors (embeddings)
in a machine-learned vector space. These embeddings are often evaluated on
their ability to identify similar or related words like a human would. In this
...
than either source, can represent meanings in multiple languages, and achieves
state-of-the-art performance on word similarity evaluations. In particular, its
score of $\rho = \scoreRW{}$ on an evaluation of rare words \cite{luong2013rw}
is 13.6\% higher than the previously best known system.
\end{abstract}

\section{Introduction}
Vector space models are an effective way to express the approximate meanings of
natural-language terms in a computational system. The goal is to represent words
or phrases in a vector space with hundreds of dimensions, such that the cosine
similarity between these vectors is higher when the words are more similar. This
kind of vector space has been useful in applications such as search and topic
detection, dating back to the introduction of latent semantic analysis
...
\newcite{mikolov2013word2vec}'s word2vec began to show dramatic improvements.
...
learns vector representations of words that predict their frequency of
co-occurrence. The algorithm operates on a global
word-word co-occurrence matrix $C$. Let $v$ denote the size of the vocabulary.
It trains $W$ and $\tilde{W}$, both $v \times k$ matrices that assign
$k$-dimensional vectors to each word and each context respectively, and $b$ and
...
$W$. The embeddings that GloVe learns
...
and that slight further improvements could be achieved on some evaluations by
adding both ConceptNet and PPDB, as shown in Table~\ref{eval-bigtable}.
...
\midrule
       ...
       &GloVe 840B   & $L_1$ & CN5.4     & ---      &     .512 &     .840 &      .798 &      .615 &     .774 &     .798 \\
\midrule
...
\caption{
    ...
    The row labeled {\bf L} shows results from \newcite{levy2015embeddings},
    using the entire MEN-3000 set and not our held-out test set.
    Row {\bf G} matches results from \newcite{pennington2014glove}, which we successfully reproduced using their data.
...
\toprule
Label   & Embeddings   & Norm  & Text std. & Retrofit & WS-353 [es] & MC-30 [es] & RG-65 [de] & RG-65 [fr] \\
\midrule
\bf R   & Skip-grams   & $L_2$ & ---       & UWN      &         --- &       .591 &       .603 &       .606 \\
\bf WL1 & GloVe 840B   & $L_1$ & CN5.4     & CN5.4    &        .444 &   \bf .765 &   \bf .673 &   \bf .781 \\
\bottomrule
\end{tabular}
...
\caption{
    ...
    \cite{faruqui2014retrofitting}. {\bf WL1} is our system, as also seen
    in Table~\ref{eval-bigtable}.
}
...
\caption{
    ...
    RW and MEN-3000 were evaluated on the development set here,
    not on the held-out test data.
}
...
$S$ be a $v \times v$ matrix denoting the semantic lexicon, and $W$ be a $v
\times n$ matrix denoting the original word vectors. If a word does not have a
...
association matrix: we exclude terms that appear fewer than 3 times, English
...
sample of $1/3$ of its word pairs for testing. Neither of these held-out test
sets were used in our evaluations until the code and parameters were finalized,
shortly before submitting this article. While
Table~\ref{eval-bigtable} shows our correlation with only the test data on
these evaluations, Table~\ref{eval-heldout-test} compares our performance
on development and test data.

We have labeled some of the rows of~\ref{eval-bigtable} as particular systems
that we would like to compare. System {\bf L} represents the previous state of the
art in word-similarity evaluations \cite{levy2015improving}. We did not run this
...
System {\bf G} represents a configuration of GloVe that was evaluated by
\newcite{pennington2014glove}. We ran the evaluation using their provided data,
and successfully reproduced their results.

{\bf WL1} and {\bf WL2} are two preferred configurations of our system. WL1
applies all the techniques described in this paper as it retrofits with
ConceptNet 5.4; WL2 is the same, but retrofits with a combination of PPDB and
ConceptNet 5.4. Both systems perform very well across all word-similarity
evaluations, and it is inconclusive which one is better. We will use
system {\bf WL1} for further comparison, because it is simpler.

% TODO: describe the variations

% TODO: make the eval-heldout-test table
...
so it performs much better on the rare words evaluation, even before
