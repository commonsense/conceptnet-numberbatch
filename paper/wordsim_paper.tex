...
evaluating word similarity, as shown in Table~\ref{eval-variations}.
...
Table~\ref{eval-variations} shows that we can reduce the interpolated matrix to
...
decisions we made in the code -- such as those in Table~\ref{eval-variations}
-- using only the development set, to preserve the integrity of the test set
and avoid ``overfitting via code''. We then evaluated the final ensemble, with
various pieces enabled, all at once on the held-out test data to produce the
