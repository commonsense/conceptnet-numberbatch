...
AutoExtend \newcite{rothe2015autoextend} is a system with similar methods to
...
Without CN5        &      .533 &     .543 &     .536 &     .842 &     .847 &     .844 \\
Without PPDB       &      .590 &     .598 &     .592 & \bf .858 & \bf .862 & \bf .860 \\
Without GloVe      &      .545 &     .543 &     .545 &     .807 &     .812 &     .808 \\
Without word2vec   &      .591 &     .583 &     .588 &     .857 &     .859 &     .858 \\
\bottomrule
\end{tabular}
...
here match the published results to within .001. word2vec, however, provides
downloadable vectors that are different from the ones used in its original
publication.
...
results, as seen in the {\bf St}, {\bf G}, $L_1$ row. We'll refer to this
configuration as ``Modified GloVe''. The fact that Modified GloVe performs
better than GloVe 42B, GloVe 840B and many other systems, even before
retrofitting any additional data onto it, highlights the unexpectedly large
benefit of lemmatization: some of the improvements from this paper can be
realized without introducing any additional data.
...
than the previously published systems, with $p < .01$. It may be the
best-performing system of all the configurations, but the ensemble of
everything but PPDB was very close to it in performance. We conclude that our
ensemble, with or without PPDB, is an improvement on the state of the art, and
that it's inconclusive what the best particular configuration of the system is.
...
code afterward. The data files are available at [{\em TBD}] and can be
version-controlled along with the code using {\tt git-annex}.
