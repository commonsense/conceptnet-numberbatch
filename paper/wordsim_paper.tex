...
the columns of the GloVe matrix often provides a notable increase in performance.
...
original word vectors. If a word does not have an corresponding word vector,
it is assigned the zero vector. Retrofitting generates a $W^n$ that minimizes
...
Where $alpha_i$ acts as weight vector. In our tests, we let
...
$$
q^{k+1} = \left( \beta W^k + \alpha^T W^0 \right)
\oslash \left( \vec{1} + \alpha \right)
$$

where $\oslash$ denotes element-wise division and $\vec{1}$ is a vector of all
ones.
